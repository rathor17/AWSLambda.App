...
WORKDIR /src
COPY ["AWSLambda.Calc.csproj", "AWSLambda.Calc/"]
RUN dotnet restore "AWSLambda.Calc/AWSLambda.Calc.csproj"

WORKDIR "/src/AWSLambda.Calc"
COPY . .
RUN dotnet build "AWSLambda.Calc.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSLambda.Calc.csproj" \
...
Execute unit tests
```
cd "AWSLambda.Calc/test/AWSLambda.Calc.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AWSLambda.Calc/src/AWSLambda.Calc"
